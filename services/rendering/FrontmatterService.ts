...
export class FrontmatterService {
	static normalizeMoodProps(
		input: Record<string, any>,
		userName: string
	): MoodData {
		const moodEmoji = this.extractMoodEmoji(input);
		const sentiment = this.extractSentiment(input);
		const dominantEmotions = this.extractStringArray(
			input.dominant_emotions
		).slice(0, MOOD_LIMITS.DOMINANT_EMOTIONS);
		const tags = this.extractStringArray(input.tags).slice(
			0,
			MOOD_LIMITS.TAGS
		);
		const people = this.extractPeoplePresent(input, userName).slice(
			0,
			MOOD_LIMITS.PEOPLE_PRESENT
		);
	...
	static upsertFrontmatter(editor: Editor, data: MoodData): void {
		...
		if (this.hasFrontmatter(frontmatterBounds)) {
			this.updateExistingFrontmatter(
				editor,
				lines,
				frontmatterBounds.end,
				serializedData
			);
		} else {
	...
	private static extractPeoplePresent(
		input: Record<string, any>,
		userName: string
	): string[] {
		...
		const excludeNames = new Set([
			...EXCLUDED_NAMES,
			(userName || "you").toLowerCase(),
		]);
		return peopleRaw.filter((person) => !excludeNames.has(person));
	}

	private static hasFrontmatter(frontmatterBounds: {
		start: number;
		end: number;
	}): boolean {
	...
	private static isDefaultValue(key: string, value: string[]): boolean {
		...
		return (
			this.isDefaultMoodEmoji(key, singleValue) ||
			this.isDefaultSentiment(key, singleValue)
		);
	}
	...
	private static removeMoodKeysFromFrontmatter(
