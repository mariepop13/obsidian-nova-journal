...
import { ensureBottomButtons, ensureUserPromptLine, removeAnchorsInBlock, typewriterInsert } from './NoteEditor';
import type { ButtonPosition, ButtonStyle } from '../../settings/PluginSettings';
...
export class ConversationResponseService {
  ...
  static async insertAfterLine(config: ResponseInsertionConfig, line: number): Promise<void> {
  ...
  static async insertAtEndOfNote(config: ResponseInsertionConfig, label: string): Promise<void> {
